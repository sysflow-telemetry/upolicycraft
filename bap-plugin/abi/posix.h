...
void *        __go_new(void *p, size_t size);
void *        __go_new_nopointers(void *p, size_t size);

// stdlib.h

char *        setlocale(int category, char *locale);
size_t        mbstowcs(char* dest, const char * src,
                       size_t n);

size_t        wcslen(char *s);
int           wcscmp(char *wcs1, char *wcs2);
char *        wcscpy(char *dest, char *src);
char *        wcscat(char *dest, char *src);
char *        wcschr(char *s, char c);
...
void          *mmap64(void *addr, size_t length, int prot, int flags, int fd, int offset);
...
int     accept4(int fd, struct sockaddr *addr, socklen_t *len_ptr, int flags)
    __attribute__((warn_unused_result));
