...
;; bap:arg-randomizer can be commented out for more deterministic behavior.

(defsystem upc:microexecutor-base
  :description "The base system for microexecution systems for uPolicyCraft."
  :depends-on (bap:terminating-stubbed-executor)
  :components (bap:greedy-scheduler
               bap:incident-location-recorder
               bap:mark-visited
...
(defsystem upc:promiscuous-executor
  :description "Run the BAP promiscuous-executor to record syscall arguments in library functions."
  :depends-on (upc:microexecutor-base)
  :components (bap:promiscuous-path-explorer))
