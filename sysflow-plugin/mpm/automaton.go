...
//TranslateOperation adapts a SysFlow OperationName to a MPM Operation
...
func (b *EffectGraph) findNode(state string) *Constraint {
...
func ParseSecurityAutomaton(model EffectGraph) *SecurityAutomaton {
	var sa = new(SecurityAutomaton)
	...
	for _, c := range model.Constraints {
		if c.Node == sa.Initial {
			...
			fileAllowList := []string{
				...
				"/proc/self/fd/[0-9]+"}
			sa.FileAllowList = append(sa.fileAllowList, fileAllowList...)
			break
		}
	}
...
func (s *SecurityAutomaton) ReportIncident(event string, r *engine.Record, out func(r *engine.Record)) {
	...
	case sfgo.TyNFStr:
		fallthrough
	case sfgo.TyPEStr:
		msg = fmt.Sprintf("Effect graph forbids %s from this state", event)
	case sfgo.TyFEStr:
		fallthrough
	case sfgo.TyFFStr:
		path := engine.Mapper.MapStr(engine.SF_FILE_PATH)(r)
		msg = fmt.Sprintf("Effect graph forbids %s on %s", event, path)
	}

	rule := engine.Rule{
		Name:     "uPolicyCraft Safety Policy",
...
func (s *SecurityAutomaton) AddObservation(r *engine.Record) {
	...
	if ty == sfgo.TyFFStr && (RegexpArrayMem(s.FileAllowList, path) || path == "") {
		logger.Trace.Printf("\nSkipping allowed file: %s", path)
		return
	}
...
// TypeCheckTrace checks whether the run of flows given in the history can inhabit the model.
// A run of flows inhabits the model if every item in the flow can be used to traverse the program's
// automaton.
func (s *SecurityAutomaton) TypeCheckTrace(out func(r *engine.Record)) {
	...
	// Uniquely identify a sysflow record by its timestamp.
	...
	for op, obs := range s.Capabilities {
		for _, ob := range obs {
			if !ob.Used {
				logger.Trace.Printf("\nSecurity Violation! Did not use:\n\t%s", op)
				r := ob.Record
				ty := engine.Mapper.MapStr(engine.SF_TYPE)(r)
