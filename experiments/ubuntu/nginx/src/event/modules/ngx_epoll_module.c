    ...
    for (i = 0; i < events; i++) {
        ...
        if ((revents & EPOLLIN) && rev->active) {
            ...
            if (flags & NGX_POST_EVENTS) {
                ...
                uids_log("Handling event");
                rev->handler(rev);
            }
        }

        uids_log("Checking w events");
        ...
        if ((revents & EPOLLOUT) && wev->active) {
            ...
            if (flags & NGX_POST_EVENTS) {
                uids_log("Posting w event");
                ngx_post_event(wev, &ngx_posted_events);
            } else {
                uids_log("Posting w handler");
                wev->handler(wev);
            }
        }
    }
