...
main(int argc, char *const *argv)
{
    ...
    if (init_cycle.pool == NULL) {
        uids_log("Could not create pool!");
        return 1;
    }
...
ngx_add_inherited_sockets(ngx_cycle_t *cycle)
{
    ...
    uids_log("After getenv");
    if (inherited == NULL) {
        return NGX_OK;
    }

    uids_log("Before log getenv");
...
ngx_set_environment(ngx_cycle_t *cycle, ngx_uint_t *last)
{
    ...
    uids_log("Before ngx_get_conf");

    ccf = (ngx_core_conf_t *) ngx_get_conf(cycle->conf_ctx, ngx_core_module);

    uids_log("Before after");
...
tz_found:
    ...
    for (i = 0; i < ccf->env.nelts; i++) {
        ...
        uids_log("Before ngx_os_environ");
        uids_debug(ngx_os_environ);
        ...
        uids_log("After ngx_os_environ");
    }

    uids_log("After nelts");
    ...
    uids_log("returning from set environment");
...
static char **my_environ = {"PATH=/usr/bin","PWD=/usr/local/nginx/", NULL};
